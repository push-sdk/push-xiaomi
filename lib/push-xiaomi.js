...
class Xiaomi {
  constructor(options = {}) {
    ...
    if(!options.appId) throw new Error('Xiaomi AppId 不能为空');
    if(!options.appSecret) throw new Error('Xiaomi appSecret 不能为空');
    if(!options.appPkgName) throw new Error('Xiaomi appPkgName 不能为空');
  }
